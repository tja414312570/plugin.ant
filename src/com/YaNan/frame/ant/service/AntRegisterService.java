...
public class AntRegisterService {
	 ...
	 * 实例
	 */
	private static AntRegisterService antClientRegisterService;
	/**
	 * 注册队列
	 */
	...
	public static AntRegisterService getInstance() {
		if(antClientRegisterService == null) {
			synchronized (AntRegisterService.class) {
				if(antClientRegisterService == null) {
					antClientRegisterService = new AntRegisterService();
				}
			}
		}
	...
	public void check() {
		...
		synchronized (waitConnectionList) {
			...
			handler = iterator.next();
				try {
					//完成连接
					AntRuntimeService.getAntRuntimeService().executeProcess(handler);
					//加入到注册检测集合
					waitRegisterList.add(handler);
					//重置注册时间
					handler.setRecoderTime(System.currentTimeMillis()+AntContext.getContext().getContextConfigure().getCheckTime());
					iterator.remove();
				} catch (Exception e) {
					iterator.remove();
