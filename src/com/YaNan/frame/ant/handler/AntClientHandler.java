...
public class AntClientHandler {
	 ...
	 * @param socketChannel
	 */
	public AntClientHandler(SocketChannel socketChannel) {
		/**
		 * 当前执行器的创建时间
		 */
		this.clientTime = System.currentTimeMillis();
		this.id = AntRuntimeService.getAntRuntimeService().getClientId();
		//设置当前线程
		...
		//设置消息处理器的缓冲区
		AntContextConfigure config = AntContext.getContext().getContextConfigure();
...
//				logger.debug("accept message from '"+this.getRemoteAddress()+"':\":"+message);
				AntRuntimeService.getAntRuntimeService().execute(new MessageProcesser(message, this));
			}
...
//		logger.debug("write message to '"+this.getRemoteAddress()+"':"+message);
	...
	public void registTimeout() {
		this.close(new AntInitException("regist timeout"));
	}
