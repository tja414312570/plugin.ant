package com.yanan.framework.ant.service;
...
import com.yanan.framework.ant.exception.AntClientHandlerOverFlow;
...
import com.yanan.framework.ant.exception.ServiceNotRunningException;
...
import com.yanan.framework.ant.protocol.ant.AntClientService;
import com.yanan.framework.ant.proxy.AntProxyMapper;
import com.yanan.framework.ant.utils.MessageProcesser;
import com.yanan.framework.ant.utils.MessageQueue;
...
public class AntRuntimeService {
	...
	public void start()  {
		...
		try {
			...
			try {
				...
				PlugsFactory.getInstance().addPlugininDefinition(AntClientService.class);
				clientService  = PlugsFactory.getPluginsInstance(ClientService.class, this);
			}
			this.discoveryService = PlugsFactory.getPluginsInstance(AntDiscoveryService.class);
			//设置运行时
			...
			AntProxyMapper proxy = PlugsFactory.getPluginsInstance(AntProxyMapper.class,this,ResourceManager.classPaths());
			proxy.execute();
	...
	public void tryRecoveryServiceAndNotifyDiscoveryService(AntServiceInstance handler) {
		...
		lock = ObjectLock.getLock(serviceName.intern(), this.getContextConfigure().getTimeout());
		if(lock.isLock())
			return;
		lock.tryLock();
		//获取当前handler的id
